    ...
    <div class="col-sm-12">
        ...
        <div class="card mt-3">
            <div class="card-body">
                ...
                <form class="forms-sample" action="{{ route('kelas.update', $item->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="jenjang" id="jenjang" class="jenjang w-100">
                                    <option value="X" @if(old('jenjang', $item->jenjang) == 'X') selected @endif>X - Sepuluh</option>
                                    <option value="XI" @if(old('jenjang', $item->jenjang) == 'XI') selected @endif>XI - Sebelas</option>
                                    <option value="XII" @if(old('jenjang', $item->jenjang) == 'XII') selected @endif>XII - Dua Belas</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <select class="guru w-100" id="guru" name="wali_kelas" required>
                            @foreach ($items as $item2)
                                <option value="{{ $item2->nip }}" @if(old('wali_kelas', $item->wali_kelas) == $item2->nip) selected @endif>{{ $item2->user->nama }} - {{ $item2->nip }}</option>
                            @endforeach
                        </select>
                    </div>
